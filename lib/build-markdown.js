...
const replaceBlogPlaceholders = (sourceFilePath, source) => {
  ...
  return source
    .replace('{{ blog-list-years }}', _match => {
      ...
      const yearIndexFilePaths = listFiles(`${constants.pages.src}/blog`)
        .filter(filePath => filePath.match((/\/blog\/[0-9]{4}\/index\.md/u)))  // 年ごとのインデックスファイルのみに絞り込む
        .filter(filePath => {  // 未来年は除外する
          const blogYear = Number(filePath.match((/\/blog\/([0-9]{4})/u))[1]);
          return blogYear <= jstNow.jstCurrentYear;  // 今年以前のみ
        })
        .sort()
        .reverse();  // 新しい順にする
    ...
    .replace('{{ blog-list-months }}', _match => {
      ...
      const monthIndexFilePaths = listFiles(`${constants.pages.src}/blog/${currentYear}`)
        ...
        .filter(filePath => {  // 今年の未来月は除外する
          if(currentYear < jstNow.jstCurrentYear) return true;  // 去年以前は全部通す
          const blogMonth = Number(filePath.match((/\/blog\/[0-9]{4}\/([0-9]{2})/u))[1]);
          return blogMonth <= jstNow.jstCurrentMonth;  // 今月以前のみ
        })
        .sort()
        .reverse();  // 新しい順にする
      // `/blog/` からのルート相対パスで Markdown リスト・リンクを組み立てる
...
const build = (sourceFilePath, source) => {
  ...
  frontMatter.canonical = `${constants.protocol}${constants.host}${rootPath}`;  // NOTE : Front Matter の `title` プロパティにミスがあるとココでつまづく
