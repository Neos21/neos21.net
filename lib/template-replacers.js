...
/** テンプレートファイルの内容 */
let template = fs.readFileSync(path.resolve(__dirname, `../${constants.templates.src}`), 'utf-8');

/** テンプレートファイルの変数を再読込する (テンプレートファイルの更新に備える) */
export const reloadTemplate = () => {
  template = fs.readFileSync(path.resolve(__dirname, `../${constants.templates.src}`), 'utf-8');
};
