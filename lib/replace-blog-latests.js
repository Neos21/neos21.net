...
/**
 * ソース中に `{{ blog-list-latests 【num】 }}` のプレースホルダがあれば置換する
 * 
...
module.exports = function replaceBlogLatests(source, prefix, mapFn, suffix, defaultNum) {
  ...
  return source.replace((/{{ blog-list-latests ([0-9]*) }}/), (_match, num) => {
    // 異常値はデフォルト値にしておく
    if(num == null || num <= 0) {
      console.warn(`Warning : {{ blog-list-latests 【num】 }} Placeholder Has Wrong Number : [${num}]`);
      num = defaultNum;
    }
