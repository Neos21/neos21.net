...
今回は _VirtualBox_ を簡単に扱えるようにしてくれる **Vagrant (ベイグラント)** というツールを使って、より簡単に Windows 上に Linux 環境を構築してみようと思う (Vagrant は VirtualBox 専用ではないが、VirtualBox を組み合わせて使用するのがデフォルトになっている)。
