...
__cordova-background-geolocation-lt__ という Cordova プラグインは、位置情報を取得し、任意のサーバに情報を送信できるプラグインだ。このプラグインの特徴はなんといっても、_アプリがバックグラウンド動作に切り替わっても位置情報の取得・送信を続ける_ところであろう。名前のとおり、Background でも Geolocation による LT : Location Tracking を行う、ということだ。
