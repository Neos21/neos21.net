...
共通的な Service だからといって、何でもかんでも `app.module.ts` に突っ込んでしまうと、`app.module.ts` が肥大化してしまい、見通しが悪くなる。Angular には _NgModule_ というモジュール分割の機能があるので、コレを使って、アプリ内の機能ごと ≒ ディレクトリごとに NgModule を作ってやろう。
