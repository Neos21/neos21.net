...
__TL;DR: ディレクトリを移動したら、Shift を押しながら iTunes を起動し、`iTunes Library.itl` ファイルのパスを再指定する。__
...
iTunes を「終了」させたら、_「任意のディレクトリ」_部分からゴッソリと別ドライブに移動させてしまう。

そしたら _`Shift` キーを押しながら iTunes を起動する。_すると「iTunes ライブラリを選択」というダイアログが最初に表示されるので、ココから移動先の `iTunes Library.itl` ファイルを指定する。
