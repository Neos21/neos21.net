...
画像ファイルを開いたりする時にこの機能を時々使うのだが、デフォルトでは__15個以上のファイルを選択すると、右クリックメニューに「開く」項目が表示されなくなってしまう__のだ。この動作は大量のファイルを誤って開かないようにと Windows 側で制限がかかっているらしい。
...
1. `regedit` (レジストリエディタ) で __`コンピューター\HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\`__ に移動する
2. 「新規」→「DWORD (32ビット) 値」を作成する
3. キーの名前を「`MultipleInvokePromptMinimum`」にする
4. 「`MultipleInvokePromptMinimum`」キーをダブルクリックして値を編集する。_「10進数」_を選択し、「値のデータ」に `15` 以上の任意の整数値を入力する
