...
フォームに入力した秒数とメモは、「Start」ボタンを押したタイミングなんかで _URL クエリパラメータ_に組み立てられる他、LocalStorage にも保存できる。**クエリ文字列で情報を保存・読み込み**できるようにしておくと、色んな筋トレメニューを_ブラウザのブックマーク_で分けて扱えるので、クエリ文字列を使う簡易アプリを作るのが個人的な趣味になってる。
