...
`text-decoration-color` による線の色だけ変更する技や、`text-decoration-style` による `border-style` チックな指定方法は知っていたが、_`text-underline-position`_ と __`text-underline-offset`__ というプロパティは今回初めて知った。__下線の位置を詳細に調整できるプロパティだ。__
