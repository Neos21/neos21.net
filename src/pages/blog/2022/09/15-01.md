...
この時はよく分からないまま触っていたのだが、COM オブジェクトである __SAPI__ (Windows Speech API)、もしくは .NET の _Speech Synthesizer_ を使うことで、Windows でも MacOS の `say` コマンドのように、任意のテキストを発話させられるワケだ。
