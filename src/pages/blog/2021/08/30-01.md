...
最近気付いたのだが、__Mac の VSCode 上のファイルツリーでファイルやディレクトリを作成すると、NFC 形式で扱われるようになったっぽい__。ちょっと詳しい情報がないので、もしかしたら `.gitconfig` で
...
で、今回は__カレントディレクトリ配下のファイルやディレクトリの名前に NFD 文字が含まれているかどうかをチェックするワンライナー__を探したので、それを紹介する。
