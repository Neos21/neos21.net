...
「ファイルの一括リネーム」を実現する、_`rename`_ というドンズバなコマンドが存在する。しかしこのコマンド、よくある名前なので混沌としていて、OS (導入環境) によって__全く別のコマンド__がインストールされる。以下、`--help` オプションの出力内容を比較してみよう。
...
続いて、リネーム後のファイル名文字列を `sed` で作る。__`p` スクリプトコマンド__を使うと、「処理前・(改行)・処理後」の順で出力できるので、コレを使う。
