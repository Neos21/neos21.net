...
こうした Pod 群を一括削除したい時に、ランダムな文字列部分を都度指定するのは面倒だ。そこで、次のようなコマンドで、__`myapp` を含む Pod を一括削除__してみる。
...
次に __`grep myapp`__ 部分で、`myapp` 部分を含む Pod 名のみに絞り込む。コレだけ実行すると次のような出力になる。
