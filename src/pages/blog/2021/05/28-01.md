...
昨日に引き続き、_AVFoundation_ を利用して動画を撮影する iOS アプリの Swift コード片の拡充。
...
また、_`videoDevice.isSmoothAutoFocusEnabled`_ に `true` を指定すると、オートフォーカスの速度をデフォルトよりも緩やかにしてくれる。シャッ、シャッと素早くピント合わせされる絵面は案外鬱陶しくて見づらいので、AF 速度を若干落とし、スムーズにピント合わせされるようにするワケだ。効き目のほどは劇的に変わるワケではないが、一応入れておく。
...
今回は面倒臭いので、__一律で5秒おきに `autoFocus` モードに戻す (`autoFocus` モードで AF が完了したら自動的に `locked` になる)__ というタイマーを設定してみようと思う。それが以下のタイマー宣言部分だ。
