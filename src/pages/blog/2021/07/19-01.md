...
1つ目は _Cloudinary_ というサイト。

- Free Plan だと 25 Monthly Credits というクレジットが貰える
- このクレジット内で、25K の「変換」を行うか、_25GB 分のストレージ_容量に使うか、25GB 分のネットワーク帯域に費やすか、をやりくりする感じ
...
2つ目は __ImageKit.io__ というサイト。

- 1ヶ月 20GB までアップ可能
- _容量上限は 20GB_ と書いてあったり、Free Storage と書いてあったりするので微妙だが、とりあえず 20GB 分は確実に大丈夫そう
- Cloudinary にはリクエスト数と変換回数に制限があったが、__ImageKit.io には制限がない__
...
とりあえず画像と動画をホスティングしたくて1つサービスを選ぶなら、__ImageKit.io__ に分がありそうだ。_Cloudinary_ は無料でもう少し容量をまかなうために ImageKit.io と併用するとか、独自の変換機能を利用したい場合は検討しても良いだろう。
