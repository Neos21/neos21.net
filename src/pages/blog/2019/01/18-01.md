...
このようなオブジェクトの配列を並び替えて、__会社名順 → 製品名順__に並び替えたい、というのが今回の趣旨。
...
ココで、`sort()` の第1引数に_独自の比較ロジック_を関数で与えると、並び替えのやり方を変えられる。
...
さて、今回やりたいのは、_「まず会社名別にソートして、1つの会社内の製品もソートする」_という要件だ。コレを実現するには、以下のように実装する。
