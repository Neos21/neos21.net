...
上のデモページの「チェック 1」ボタンを押下すると、`window.getComputedStyle()` を使って、ある要素の `font-family` 値を取得する。対象の要素の CSS は、`font-family: "デタラメゴシック", "游ゴシック";` と指定されており、__`window.getComputedStyle()` は `"デタラメゴシック", "游ゴシック"` という文字列を取得してしまう。__
