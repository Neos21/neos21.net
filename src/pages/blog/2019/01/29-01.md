...
特徴的なのは、__配列変数の宣言時にアットマーク `@` を使う__点。変数を見るだけで配列だなと分かるのはありがたい。
...
`opendir`・`readdir`・`closedir` は組み込みの関数。__関数呼び出しはカッコ `()` を付けても付けなくても呼べる。__
...
> 関数をロジックの前置きに定義すると、カッコなしでもＯＫですが、そうしない。__『()』小カッコを付けてユーザ定義関数を明示する。__
> 
