...
- 事後条件 (Postcondition) : _`return` する戻り値の仕様チェック。_
...
TypeScript の場合、「その型宣言なら `null` はチェックせずとも弾けるのでは？」といった指摘はありうるが、_実行時に TypeScript での型チェックが働くワケではない_ので、API から取得したデータを元に動的に処理したりしているような場面で意図した型で動作しない場合もある。あくまで擬似コードとして、言語別の個別の指摘は無視する。ココでは「何をすることが事前条件と言われるのか」といったことを押さえるために見て欲しい。
