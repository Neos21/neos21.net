...
例外を捕捉するための `catch` 句の中で例外が発生している、という可能性は、最初はなかなか思い当たらないだろう。しかし、おかしな動きをしている時は、「ココでキャッチしている**はず**だから…」といった思い込みを捨て、_`catch` 句の中に問題はないか_と調べる癖を付けたい。

## 「ログ出力処理」そのものが悪影響を及ぼす

また、この例で厄介なのは、_エラーの内容を把握するために仕込んだはずのログ出力処理自体に問題_があって、エラーの詳細が表に現れなかったところだ。

「正常ログが出る_はず_」「異常ログが出る_はず_」と思っているのに、想定していたどちらのログも出なかった場合は、**ログ出力処理部分に問題はないか**をチェックするようにしたい。
