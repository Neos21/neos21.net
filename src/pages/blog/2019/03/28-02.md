...
- そのリソースをどこまで操作できるようにするか、という権限設定が **Policy (ポリシー)** で、Policy の適用範囲は _Group (グループ)_ 単位などで指定できる。
...
OKE クラスタで、OCIR 上の Docker イメージを使えるようにするには、_Docker Registry Secret_ という**クレデンシャル情報 (秘密情報)** を Kubernetes に登録する必要がある。秘密情報は開発マシンに保存されるのではなく、Kubernetes クラスタ内に保持される。
