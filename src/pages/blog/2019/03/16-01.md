...
JDK には __`cacerts`__ というファイルが含まれていて、Java プログラムが動作する時に参照する「ルート認証局」の証明書が管理されているようだった。
...
__キーストアのパスワードは、デフォルトでは `changeit`__ になっているので、コレを入力する。すると大量のデータが出力される。
...
そこで、_`grep` で `発行者` の文字を含む行だけ抽出し、`sort` コマンドで順序を直してみる。_
