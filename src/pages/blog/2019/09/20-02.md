...
続いて tmux。コチラもホームディレクトリ直下に **`.tmux.conf`** というファイルを作り、以下の内容をコピペする。_MacOS でも Windows GitSDK でも動作するよう調整してある。_
...
- Prefix を `Ctrl + t` にした。別途 _CapsLock キーを Ctrl (Control) キーに変更_しておくと、**左手の位置を大きく動かさずに押せる**ようになるのでオススメ
