...
今回は、GAS から Twitter 投稿を行うためにライブラリを追加する。以下で紹介されている __TwitterWebService__ というライブラリだ。
...
- 「プロジェクトバージョン」プルダウン : __「New」__を選ぶ
...
3. _予め自作 Twitter アプリに設定しておいたコールバック URL_ に遷移する
...
このコールバック URL が用意できたら、Twitter の App 設定を開き、_「Callback URLs」_欄にコピペする。
...
実行後、画面上は何の変化もないと思うので、_「表示」メニュー → 「ログ」_と選択する。すると、ログの中に以下のような URL が出力されているかと思う。

- `https://api.twitter.com/oauth/authorize?oauth_token=XXXXXXXXXXXXXXXXXXXXXXXXXXX`

__この URL をコピペしてブラウザで遷移__すると、Twitter の認証画面が表示されるので、ログインして認証する。
