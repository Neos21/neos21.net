...
このように、__`npm` コマンドを実行したい場合は `docker-compose run` を使用する__ことで、ローカルに `npm` コマンドをインストールした時と同様に作業できる。
...
ファイルの準備ができたら、サーバを起動して動作確認してみよう。ココで _`docker-compose up`_ を使うことになる。 - サーバを起動して動作確認する
