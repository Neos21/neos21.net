...
そこで見つけたのが、__Vue-CLI-Plugin-Express__ という Vue CLI 用のプラグイン。既存の Vue CLI ベースのプロジェクトに、Express サーバを統合してくれるというモノだ。早速使ってみよう。
...
を実行してみよう。コチラは元々の Vue CLI が提供する、フロントエンド環境の簡易サーバが立ち上がる。ポート番号は先程の _3000_ 番ではなく __8080__ 版で立ち上がっていることに留意。
