...
今回はコレを使って、__ブックマークレット形式__で呼び出せる Function と、_Slack の Slash Command_ として呼び出せる Function を2つ作ってみた。
...
環境変数は、Netlify 管理画面から「Settings」→「Build & deploy」→「Environment」と移動し、_「Environment variables」_欄にて設定できるので、ココに各種トークンを入れておく。
