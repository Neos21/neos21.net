...
上述の公式アナウンスのページを見てみると、コレまで使ってきた Vue CLI (`@vue/cli`) ではなく、Vue.js の作者が作った __Vite (ヴィート)__ というツールを使用する方法が紹介されていた。
...
まずは、公式アナウンスの記載に従って _`npm init vite-app`_ を叩いてみよう。
...
`package.json` の中身はこんな感じ。_依存パッケージ少ない！_
...
`main.js` 内にある _`import './index.css'`_ の行を __`import './index.scss'`__ と変更したところ、`npm run dev` していたコンソールに
...
こうした型チェックはどうしたらいいのかというと、__ビルド時にやるんじゃなくて、IDE で開発中に検知しろ__ってことみたい。確かにね。
