...
クエリパラメータではなく、`example/1` のように、ID などが URL にそのまま付くタイプのモノを_ルートパラメータ_という。
...
__Matrix URI (マトリクス URI)__ とは、セミコロン `;` を使ってパラメータを表記するモノ。`?` と `&` による_クエリパラメータ_のようなモノだが、以下のような違い、メリットがある。
...
ルートパラメータ、Matrix URI パラメータを受け取る `paramMap` ではなく、_`queryParamMap`_ で受け取る。
...
ページ内リンクにも使う `#anchor` なヤツ。_Fragment (フラグメント)_ と呼ぶ。
