...
Angular In Memory Web API の標準仕様では、前述の `/users` のように、配列形式でしかデータを取得できないはずだ。しかし、_`responseInterceptor()`_ というメソッドをオーバーライドすると、**特定のパスへのアクセス時にレスポンスデータをカスタマイズ**できるのだ。
