...
なお、__`Main.storyboard` 全体の背景色を変更するには__、`LaunchScreen.storyboard` で紹介したときと同様。`Main.storyboard` を開いたら、_「View Controller Scene」→「View Controller」→「View」を選択し、右ペインの「Attributes Inspector」から「Background」を選択_して色を変更できる。
